...
{
    /// <summary>
    /// ProductWorkingStatisticsData业务逻辑层，可使用显示事物
    /// </summary>
    public class ProductWorkingStatisticsDataBLL : MyClassBase
    {
        DAL.ProductWorkingStatisticsDataDAL dal = new DAL.ProductWorkingStatisticsDataDAL();
        ...
        public virtual IEnumerable<ProductWorkingStatisticsData> SelectByProductIdTherapyModeDataTime(Guid productId, int therapyMode, DateTime startTime, DateTime endTime)
        {
            ...
            try
            {
                ...
                {
                    return dal.SelectByProductIdTherapyModeDataTime(productId, therapyMode, startTime, endTime);
                }
            }
            catch (Exception ex)
            {
